...
Atomically switch `current` and start the web service. Its `Type=simple` systemd start can return before Gunicorn has bound its listener, so a single immediate probe is not a readiness result. Verify the selected release and candidate database revision as hard gates, then poll the actual running unit, expected loopback-only listener, and local `/healthz` through the configured one-hop proxy boundary until all pass or a documented 60-second monotonic deadline expires. A failed unit or invalid release/schema fails immediately; transient startup absence is retried at short intervals without extending the deadline. Keep public ingress closed throughout this wait. On timeout, record failure and leave Caddy and timers stopped. Only after local readiness passes may the workflow persist the conservative public-reopening state, start Caddy, and verify the configured HTTPS hostname and `/healthz`; then allow a bounded observation check of application and Caddy state. Only then present the recorded timer state and catch-up decision for operator-controlled resumption. Do not start the daily service as a smoke test: it can scrape, upload a backup, and send member messages.
...
If activation fails before public ingress reopens and no schema migration occurred, switch `current` back and restart the prior web service. Apply the same 60-second monotonic, bounded local-readiness polling to the previous release before recording public reopening or starting Caddy; do not treat `systemctl start` or an immediate missing listener as a final result. If the previous service fails or the deadline expires, retain the previous release link and live database, record the failed rollback, and leave ingress and timers closed for operator recovery. Once local readiness succeeds, persist the conservative public-reopening state before starting Caddy, verify public HTTPS health, and leave timers paused until the operator's catch-up decision. If migration occurred, the previous release is not assumed compatible: use the existing validated `restore-snapshot --confirm-stopped` procedure only after confirming that no public writes were accepted, or activate a reviewed compatible release. If public traffic has resumed, never automatically replace the database with the predeployment snapshot because that would discard accepted changes. Apply the existing privacy re-clean and restoration checks before reopening a historical snapshot.
...
- [A `Type=simple` start races Gunicorn readiness] -> Use a short-interval monotonic readiness poll with a fixed 60-second deadline for candidate activation and compatible rollback; keep ingress and timers closed until verified health and fail closed on timeout.
