...
### Requirement: Production has explicit retention settings

Before public launch, a responsible club officer SHALL choose cleanup periods for expired
authentication and abuse records and unapproved registrations, and SHALL check how
active persons, assignment audits, logs and backups are retained. The executable
policy contains only the cleanup periods and batch size; a detailed per-class
questionnaire is not a technical gate. The public privacy notice SHALL describe
actual processing and enabled providers.

#### Scenario: Retention settings are reviewed for launch

- **WHEN** the privacy launch gate is evaluated
- **THEN** all cleanup periods have valid values and the club has considered backup retention
- **AND** unresolved placeholders block launch
...
The club-reviewed schedule SHALL document that assignment audit entries are append-only application records and may retain historical name snapshots after a roster record is deactivated or deleted. Routine authentication and registration cleanup SHALL NOT alter or delete assignment audit entries. The operator and responsible club officer SHALL explicitly approve the audit purpose, access, retention period and handling of a valid deletion or restriction request; any requirement to redact or remove audit data SHALL be handled by a separately reviewed change that preserves audit integrity and readability rather than an ad hoc database edit.
...
### Requirement: Third-party processing is reviewed by the club

Before public launch, a responsible club officer SHALL review the configured e-mail,
SMS, Dropbox and hosting providers and ensure the public privacy notice reflects
actual processing. Provider contracts and incident contacts may be kept in the
club's records; the application SHALL NOT require a provider questionnaire in JSON.
Monitoring SHALL not transfer personal data when local status is sufficient.

#### Scenario: Provider review before launch

- **WHEN** the club reviews the public privacy notice
- **THEN** it compares the notice with enabled e-mail, SMS, Dropbox and hosting services
- **AND** the launch decision remains no-go until discrepancies are corrected
