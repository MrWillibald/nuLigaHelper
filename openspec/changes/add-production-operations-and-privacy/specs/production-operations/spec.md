...
Production SHALL record one responsible club officer, review date and explicit
go/no-go decision. The short launch record SHALL require a backup/restore rehearsal,
a checked privacy notice and chosen retention settings. Technical preflight SHALL
continue to check runtime, permissions, host readiness and public TLS separately.
The rollback runbook SHALL cover writer quiescing, application restoration, database
compatibility, health and duplicate-notification cautions.

#### Scenario: Launch gate is incomplete

- **WHEN** the decision is no-go or one of the three launch checks is false
- **THEN** the launch check fails

#### Scenario: Rollback is invoked

- **WHEN** a blocking production fault meets a documented rollback trigger
- **THEN** the operator checks database compatibility and health before reopening
