...
**Reason**: Ordinary game numbers are the authoritative stable identity; valid SPF
reuse is handled by collapsing a whole Spielfest into a synthetic numbered game.

**Migration**: Convert ordinary numbers to canonical textual values and assign each
stored SPF date/age-group aggregate a deterministic pseudo game number.

### Requirement: Scraped games have stable source identity
**Reason**: nuLiga meeting IDs and fallback fingerprints can change and create duplicate
rows even when the authoritative game number is unchanged.

**Migration**: Synchronize ordinary games by season and canonical game number and remove
source-key identity after reconciling existing rows.

### Requirement: Ambiguous source identity fails safely
**Reason**: Source identity is removed; ambiguity is now handled while grouping SPF rows
and migrating pre-existing duplicate ordinary game numbers.

**Migration**: Replace source-key collision checks with SPF consistency validation,
canonical-number uniqueness validation and migration reconciliation reports.

### Requirement: Game lifecycle events retain exact identity
**Reason**: The lifecycle contract now uses canonical season/game-number identity for
ordinary games and one aggregate identity for each Spielfest rather than supporting
coexisting same-season duplicate-number games.

**Migration**: Use the replacement requirement "Lifecycle events use canonical local
identity" and continue carrying local `Game.id` through every event.

### Requirement: Existing games are selected unambiguously
**Reason**: Administrative selection no longer needs to distinguish valid same-season
duplicate-number matches; it must distinguish ordinary games and collapsed Spielfeste
while continuing to mutate by internal ID.

**Migration**: Use the replacement requirement "Administrative selection uses internal
game identity" and update labels and number parsing for textual canonical values.
