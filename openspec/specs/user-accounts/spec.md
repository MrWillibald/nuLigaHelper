...
contact channel (e-mail address or phone number) and the team they wish to join. The
system SHALL obtain the registrant's consent to publish their name on the public
schedule before accepting the registration.
...
- **AND** sends a verification message to the supplied channel
- **AND** the registrant is not yet on the roster

#### Scenario: Registration without consent

- **WHEN** a visitor submits the registration form without confirming the consent notice
- **THEN** the registration is rejected with an explanatory message

#### Scenario: Registration without a contact channel

- **WHEN** a visitor submits the registration form with neither e-mail nor phone
...
- **WHEN** a visitor registers with a contact channel that already belongs to a person on
  the roster
- **THEN** the system responds exactly as it does for an unused channel
...
- **WHEN** the registrant follows the verification link or enters the verification code
...
### Requirement: Passwordless login by e-mail link or SMS code

The system SHALL authenticate a person by proving control of a contact channel already
stored on their record: a single-use link sent by e-mail, or a single-use numeric code
sent by SMS. The system SHALL prefer e-mail and fall back to phone, matching the delivery
rule used for notifications. The system SHALL NOT store passwords.

#### Scenario: Login by e-mail link

- **WHEN** a person requests a login for a contact that belongs to an approved person
  with an e-mail address
- **THEN** a single-use link is sent to that address
- **AND** following the link within its validity period signs the person in

#### Scenario: Login by SMS code

- **WHEN** the person has a phone number but no e-mail address
- **THEN** a single-use numeric code is sent by SMS
- **AND** entering that code within its validity period signs the person in

#### Scenario: Login token is single use

- **WHEN** a login link or code that has already been used is presented again
- **THEN** the sign-in is refused and a new one must be requested

#### Scenario: Login token expires

- **WHEN** a login link or code is presented after its validity period has passed
- **THEN** the sign-in is refused and a new one must be requested
